...
public abstract class AbstractEntitySet<TKey, TEntity> implements EntitySet<TEntity> {
    ...
    @Override
    public final long countAll() throws Exception {
        return query().prepare().count();
    }

    @Override
    public final long countAllWhere(Condition<TEntity> condition) throws Exception {
        return query().where(condition).prepare().count();
    }

    @Override
    public final <T> TEntity findFirst(ValueField<TEntity, T> field, T value) throws Exception {
        return query().where(field.eq(value)).prepare().firstOrDefault();
    }

    @Override
    public final TEntity findFirstWhere(Condition<TEntity> predicate) throws Exception {
        return query().where(predicate).prepare().firstOrDefault();
    }

    @Override
    public final <T> TEntity[] findAllBy(ValueField<TEntity, T> field, T value) throws Exception {
        return query().where(field.eq(value)).prepare().toArray();
    }

    @Override
    public final TEntity[] findAllWhere(Condition<TEntity> predicate) throws Exception {
        return query().where(predicate).prepare().toArray();
    }

    @Override
    public final TEntity[] toArray() throws Exception {
        return query().prepare().toArray();
    }

    @Override
    public final List<TEntity> toList() throws Exception {
        return query().prepare().toList();
    }

    @Override
    public final <T> Map<T, TEntity> toMap(Field<TEntity, T> keyField) throws Exception {
        return query().prepare().toMap(keyField);
    }

    @Override
    public final <K, V> Map<K, V> toMap(Field<TEntity, K> keyField, Field<TEntity, V> valueField) throws Exception {
        return query().selectToMap(keyField, valueField);
    }

    @Override
    public final <T> Collection<T> map(Transformer<TEntity, T> mapper) throws Exception {
        return Stream.of(toList()).map(mapper::transform).collect(Collectors.toList());
    }

    @Override
    @SafeVarargs
    public final TEntity[] add(TEntity... entities) throws Exception {
        addAll(Arrays.asList(entities));
        return entities;
    }

    @Override
    public final TEntity add(TEntity entity) throws Exception {
